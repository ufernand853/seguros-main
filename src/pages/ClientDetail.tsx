...
function PolicyRow({ policy }: { policy: PolicySummary }) {
  ...
  const roleLabels = roles.length ? roles.map((role) => ROLE_LABELS[role] ?? role) : [];
      ...
      <td className="px-3 py-2">
        {roleLabels.length ? (
          <div className="flex flex-wrap gap-1">
            {roleLabels.map((role) => (
              <span key={role} className="inline-flex items-center px-2 py-0.5 rounded-full bg-indigo-50 text-xs font-semibold text-indigo-700">
                {role}
              </span>
            ))}
          </div>
